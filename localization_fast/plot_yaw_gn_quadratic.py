...
def prepare_sequence_context(sequence_id, device_name, weights_path, fov_deg, res_deg):
    import torch
    ...
    weights = weights_path or Path(root) / "model_dev/model_weights/6_deg_attentional_MSE_delauney/best.pth"
...
def compute_frame_quadratic_and_curve(
    ...
    options = ResidualBuildOptions(
        ...
        geometry_backend="torch_frustum",
        geometry_batch_size=geometry_batch_size,
        keep_diagnostics=False,
    )
    T_center = make_delta_T(rpy_deg=np.array([0.0, 0.0, center_yaw_deg])) @ frame_inputs["T_gt"]
    ...
    costs = []
    for sample_idx, yaw_deg in enumerate(yaw_samples_deg, start=1):
        print(f"    cost sample {sample_idx}/{len(yaw_samples_deg)}: yaw {yaw_deg:+.3f} deg")
        T_sample = make_delta_T(rpy_deg=np.array([0.0, 0.0, yaw_deg])) @ frame_inputs["T_gt"]
    ...
    return float(lin.cost), float(lin.g[5]), float(lin.H[5, 5]), np.asarray(costs)


def process_failed_frames(args) -> None:
    results_path = args.results_csv.expanduser().resolve()
    results_df = pd.read_csv(results_path)
    required = ["sequence_id", "frame_id", "final_yaw_deg"]
    ...
    failed = results_df[np.abs(results_df["final_yaw_deg"].astype(float)) >= args.failure_threshold_deg].copy()
    if failed.empty:
        print(f"No frames exceed |final_yaw_deg| >= {args.failure_threshold_deg}.")
        return
    ...
    for sequence_id, sequence_rows in failed.groupby("sequence_id", sort=False):
        ...
        for row_idx, row in sequence_rows.iterrows():
            ...
            center_yaw_deg = float(row["final_yaw_deg"])
            print(f"\nFrame {frame_id}: final yaw error {center_yaw_deg:+.6f} deg")
            yaw_label = format_yaw_for_filename(center_yaw_deg)
            sample_csv = output_dir / f"{frame_id}_final_yaw_{yaw_label}_deg_samples.csv"
            output_png = output_dir / f"{frame_id}_final_yaw_{yaw_label}_deg.png"

            if sample_csv.exists() and not args.recompute_costs:
                cached_df = pd.read_csv(sample_csv)
                metadata_columns = ["linearization_yaw_deg", "gn_C0", "gn_g_yaw_per_rad", "gn_H_yaw"]
                ...
                if missing:
                    ...
                    cached_center = float(cached_df["linearization_yaw_deg"].iloc[0])
                    print(f"  Using cached cost samples: {sample_csv}")
                    plot_yaw_curve_and_quadratic(
                        csv_path=sample_csv,
                        center_yaw_deg=cached_center,
                        c0=float(cached_df["gn_C0"].iloc[0]),
                        g_yaw=float(cached_df["gn_g_yaw_per_rad"].iloc[0]),
                        h_yaw=float(cached_df["gn_H_yaw"].iloc[0]),
                        output_path=output_png,
                        window_deg=plot_window_deg,
                        quadratic_window_deg=args.quadratic_window_deg,
            ...
            try:
                ...
                yaw_samples = make_yaw_samples(
                    center_yaw_deg=center_yaw_deg,
                    global_min_deg=args.global_min_deg,
                    global_max_deg=args.global_max_deg,
                    global_step_deg=args.global_step_deg,
                    local_window_deg=sampling_local_window_deg,
                    local_step_deg=args.local_step_deg,
                )
                c0, g_yaw, h_yaw, costs = compute_frame_quadratic_and_curve(
                    context=context,
...
def compute_gn_quadratic_terms_from_frame(
    ...
    if weights_path is None:
        weights_path = Path(boreas_vtr_wrapper_dir) / "model_dev/model_weights/6_deg_attentional_MSE_delauney/best.pth"
    ...
    for seq in bd.sequences:
        ...
        T_offset = make_delta_T(rpy_deg=np.array([0.0, 0.0, center_yaw_deg]))
        ...
        print("Computing yaw-only GN linearization...")
        lin = compute_radar_residual_and_jacobian_fast(
            P_v=P_v,
            T=T_center,
            odom_transforms=odom_transforms,
        ...
        return float(lin.cost), float(lin.g[5]), float(lin.H[5, 5])
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Process result rows with abs(final_yaw_deg) at or above this threshold.",
    )
    parser.add_argument(
        "--recompute-costs",
        action="store_true",
    ...
    parser.add_argument("--global-min-deg", type=float, default=-3.0)
    parser.add_argument("--global-max-deg", type=float, default=3.0)
    parser.add_argument("--global-step-deg", type=float, default=0.5)
    parser.add_argument(
        "--local-window-deg",
        type=float,
        default=None,
    ...
    parser.add_argument("--local-step-deg", type=float, default=0.05)
    parser.add_argument(
        "--window-deg",
        type=float,
        default=None,
    ...
    if args.center_yaw_deg is None:
        raise ValueError("--center-yaw-deg is required unless --results-csv is used.")
    ...
    if args.C0 is None or args.g is None or args.H is None:
        ...
        args.C0, args.g, args.H = compute_gn_quadratic_terms_from_frame(
            frame_id=args.frame,
            center_yaw_deg=args.center_yaw_deg,
    ...
    if args.output is None:
        ...
        yaw_label = format_yaw_for_filename(args.center_yaw_deg)
        output_path = output_dir / f"{csv_path.stem}_center_yaw_{yaw_label}_deg.png"
    else:
